    ...
    return (
        <div className="space-y-5 pb-12">
            <div className="max-w-2xl mx-auto space-y-5">
                <div className="flex p-1.5 bg-card border border-foreground/[0.12] rounded-2xl shadow-sm gap-1.5 w-fit mx-auto">
                    {tabs.map((tab) => (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id as OrderTab)}
                            className={`relative flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-semibold uppercase tracking-widest transition-all ${activeTab === tab.id ? "bg-blue-50 text-blue-700 border border-blue-100" : "text-foreground/45 hover:text-foreground/70"}`}
                        >
                            <tab.icon className={`w-4 h-4 ${activeTab === tab.id ? "text-blue-600" : "text-foreground/30"}`} />
                            {tab.label}
                        </button>
                    ))}
                </div>
                ...
                <AnimatePresence mode="wait">
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -8 }}
                        className="bg-card border border-foreground/[0.12] rounded-2xl shadow-sm p-6"
                    >
                        {activeTab === 'new' && (
                            <div className="space-y-5">
                                <div>
                                    <h3 className="text-base font-bold text-foreground/90">Create New Order</h3>
                                    <p className="text-xs text-foreground/45 mt-0.5">Initialize a new fulfillment request.</p>
                                </div>
                                <form className="space-y-4">
                                    <div>
                                        <label className="block text-xs font-semibold text-foreground/60 uppercase tracking-widest mb-1.5">Customer</label>
                                        <div className="relative">
                                            <input type="text" className={inputCls + " pl-10"} placeholder="Search for a customer..." />
                                            <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-foreground/30" />
                                        </div>
                                    </div>
                                    <div>
                        ...
                        {activeTab === 'track' && (
                            <div className="space-y-5">
                                <div>
                                    <h3 className="text-base font-bold text-foreground/90">Track Existing Order</h3>
                                    <p className="text-xs text-foreground/45 mt-0.5">Monitor real-time fulfillment status.</p>
                                </div>
                                <div className="flex gap-3">
                                    <div className="flex-grow relative">
                                        <input type="text" className={inputCls + " pl-10"} placeholder="Enter Tracking ID" />
                                        <Truck className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-foreground/30" />
                                    </div>
                                ...
                                <div className="py-12 border-2 border-dashed border-foreground/[0.12] rounded-2xl text-center">
                                    <div className="w-12 h-12 bg-foreground/[0.04] rounded-2xl flex items-center justify-center mx-auto mb-3 border border-foreground/[0.12]">
                                        <SearchIcon className="w-6 h-6 text-foreground/30" />
                                    </div>
                                    <p className="text-xs font-semibold text-foreground/45 uppercase tracking-widest">Courier API integration pending</p>
                                </div>
                            </div>
                        )}
                    </motion.div>
