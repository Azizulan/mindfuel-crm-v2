            ...
            <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-4 flex flex-wrap items-center gap-3">
                <div className="flex bg-gray-100 p-1 rounded-xl">
                    <button
                        ...
                        className={`px-4 py-1.5 text-xs font-semibold uppercase rounded-lg transition-all ${selectedDate === todayStr ? 'bg-white shadow-sm text-gray-800' : 'text-gray-400 hover:text-gray-600'}`}
                    >Today</button>
                    <button
                        onClick={() => onDateChange(yesterdayStr)}
                        className={`px-4 py-1.5 text-xs font-semibold uppercase rounded-lg transition-all ${selectedDate === yesterdayStr ? 'bg-white shadow-sm text-gray-800' : 'text-gray-400 hover:text-gray-600'}`}
                    >Yesterday</button>
                </div>
                <input
                    type="date"
                    ...
                    max={todayStr}
                    className="bg-gray-50 border border-gray-200 rounded-xl px-3 py-1.5 text-xs font-medium text-gray-700 outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500"
                />
                <button
                    onClick={onRefresh}
                    className="p-2 bg-gray-50 hover:bg-gray-100 text-gray-500 rounded-xl border border-gray-200 transition-all"
                    title="Refresh"
                >
                ...
                <div className="ml-auto flex items-center gap-4 text-[10px] font-semibold text-gray-400 uppercase tracking-widest">
                    ...
                    <div className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-gray-200"></span> No Activity</div>
                </div>
            </div>
                    ...
                    >
                        <div className="px-5 py-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                            ...
                            <div className="text-right">
                                <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Total Today</p>
                                <p className="text-xl font-bold text-gray-900">{agent.totalToday}</p>
                            </div>
                        </div>
                        <div className="p-5">
                                    ...
                                    .map((stat, idx) => {
                                        ...
                                        return (
                                            <div key={idx} className="flex-1 group relative flex flex-col justify-end items-center h-full">
                                                ...
                                                <span className={`text-[8px] font-semibold mt-1.5 ${isShowingToday && stat.hour === currentHour ? 'text-blue-600' : 'text-gray-400'}`}>
