  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className={`bg-white border-r border-gray-200 flex flex-col fixed inset-y-0 left-0 z-40 w-60 md:sticky md:top-0 md:h-screen md:flex-shrink-0 ${!isOpen && 'hidden md:flex'}`}
      >
        {/* Logo */}
        <div className="flex items-center justify-between px-5 py-5 border-b border-gray-100">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-4.5 h-4.5 w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="white">
                <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 0 0-1.091-.852H4.5A2.25 2.25 0 0 0 2.25 4.5v2.25Z" />
              </svg>
            </div>
            <div>
              <p className="text-sm font-bold text-gray-900 leading-none">CRM Pro</p>
              <p className="text-[10px] text-gray-400 mt-0.5">{user.role}</p>
            </div>
          </div>
          <button onClick={() => setIsOpen(false)} className="md:hidden p-1 rounded-lg text-gray-400 hover:bg-gray-100">
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
          ...
          {isAdmin && (
            <>
              ...
              <div className="pt-3 pb-1 px-3">
                <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Data</p>
              </div>
              ...
              <div className="pt-3 pb-1 px-3">
                <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Catalog</p>
              </div>
              ...
              <div>
                <button
                  ...
                  className={`w-full flex items-center justify-between gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
                    isOrdersActive ? 'bg-blue-50 text-blue-600' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-700'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <span className={isOrdersActive ? 'text-blue-600' : 'text-gray-400'}>
                    ...
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden pl-9 mt-0.5 space-y-0.5">
                      {[['newOrder', 'New Order'], ['orderStatus', 'Status'], ['trackOrder', 'Tracking']].map(([v, l]) => (
                        <button key={v} onClick={() => go(v as View)}
                          className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${activeView === v ? 'text-blue-600 bg-blue-50' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'}`}
                        >{l}</button>
                      ))}
                    </motion.div>
                  )}
        ...
        <div className="px-3 py-4 border-t border-gray-100 space-y-0.5">
          ...
          <div className="flex items-center gap-3 px-3 py-2 mt-1">
            ...
            <button onClick={onLogout} title="Logout" className="p-1.5 rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-500 transition-colors">
