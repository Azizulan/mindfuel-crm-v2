...
const FLAG_META: Record<string, { label: string; color: string }> = {
    ...
    'low-sample':     { label: 'Low sample', color: 'bg-foreground/[0.08] text-foreground/45 border-foreground/[0.12]' },
};
...
const AgentCoachingPage: React.FC = () => {
    ...
    const vsTeam = (val: number, avg: number, higherIsBetter = true) => {
        if (!avg) return 'text-foreground/85';
    ...
    return (
        <div className="space-y-5 pb-12">
            ...
            <div className="glass-surface p-4 flex flex-wrap items-center gap-3">
                <span className="text-xs font-semibold text-foreground/60 uppercase tracking-widest">Period</span>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                        ...
                        <div key={s.label} className="glass-surface p-5">
                            <p className="text-[10px] font-bold text-foreground/45 uppercase tracking-widest">{s.label}</p>
                            <p className="text-2xl font-black text-foreground mt-1 font-mono">{s.value}</p>
                            <p className="text-[11px] text-foreground/45 mt-1">{s.sub}</p>
                        </div>
                    ))}
                </div>
            )}
            ...
            <div className="glass-surface overflow-hidden">
                <div className="px-5 py-4 border-b border-foreground/[0.08]">
                    <h3 className="text-sm font-bold text-foreground/90">Agent Quality — ranked by conversion</h3>
                    <p className="text-[11px] text-foreground/45 mt-0.5">Coaching flags compare each agent to the team average (agents with ≥10 calls).</p>
                </div>
                {loading ? (
                    <div className="py-16 text-center text-foreground/45 italic">Loading…</div>
                ) : agents.length === 0 ? (
                    <div className="py-16 text-center text-foreground/45 italic">No call data for this period.</div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full text-xs min-w-[760px]">
                            <thead>
                                <tr className="border-b border-foreground/[0.08] bg-gray-50/50">
                                    <th className="text-left px-4 py-3 text-[10px] font-bold text-foreground/45 uppercase tracking-widest">Agent</th>
                                    <th className="text-right px-3 py-3 text-[10px] font-bold text-foreground/45 uppercase tracking-widest">Calls</th>
                                    <th className="text-right px-3 py-3 text-[10px] font-bold text-foreground/45 uppercase tracking-widest">Contact%</th>
                                    <th className="text-right px-3 py-3 text-[10px] font-bold text-foreground/45 uppercase tracking-widest">Positive%</th>
                                    <th className="text-right px-3 py-3 text-[10px] font-bold text-foreground/45 uppercase tracking-widest">Negative%</th>
                                    <th className="text-right px-3 py-3 text-[10px] font-bold text-foreground/45 uppercase tracking-widest">Orders</th>
                                    <th className="text-right px-3 py-3 text-[10px] font-bold text-foreground/45 uppercase tracking-widest">Conv%</th>
                                    <th className="text-right px-3 py-3 text-[10px] font-bold text-foreground/45 uppercase tracking-widest">৳/Call</th>
                                    <th className="text-left px-4 py-3 text-[10px] font-bold text-foreground/45 uppercase tracking-widest">Signals</th>
                                </tr>
                            </thead>
                                    ...
                                    >
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-2.5">
                                                ...
                                                <span className="font-semibold text-foreground/90 truncate max-w-[120px]">{a.agent}</span>
                                            </div>
                                        </td>
                                        <td className="px-3 py-3 text-right font-mono text-foreground/85">{a.totalCalls}</td>
                                        <td className={`px-3 py-3 text-right font-mono font-bold ${a.contactRate < 0.4 ? 'text-red-500' : 'text-foreground/85'}`}>{pct(a.contactRate)}</td>
                                        ...
                                        <td className="px-3 py-3 text-right font-mono text-foreground/85">{a.orders}</td>
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex flex-wrap gap-1">
                                                {a.flags.filter(f => f !== 'low-sample').map(f => (
                                                    <span key={f} className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full border ${FLAG_META[f]?.color ?? 'bg-foreground/[0.08] text-foreground/60 border-foreground/[0.12]'}`}>
                                                        {FLAG_META[f]?.label ?? f}
                                                    </span>
                                                ))}
                                                {a.flags.includes('low-sample') && (
                                                    <span className="text-[9px] text-foreground/30 italic">too few calls to rate</span>
                                                )}
                                            </div>
                                        </td>
                                    </motion.tr>
