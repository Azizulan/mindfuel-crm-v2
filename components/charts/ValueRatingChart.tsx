    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md border border-slate-200 h-[380px] flex flex-col overflow-hidden">
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h3 className="text-lg font-bold text-slate-700">Value Distribution</h3>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">Last {data.length} periods</p>
                </div>
                <div className="flex bg-slate-100 p-1 rounded-lg">
                    <button 
                        onClick={() => setView('monthly')}
                        className={`px-3 py-1 text-[10px] font-black uppercase tracking-tighter rounded-md transition-all ${view === 'monthly' ? 'bg-white shadow-sm text-amber-600' : 'text-slate-400'}`}
                    >Monthly</button>
                    <button 
                        onClick={() => setView('yearly')}
                        className={`px-3 py-1 text-[10px] font-black uppercase tracking-tighter rounded-md transition-all ${view === 'yearly' ? 'bg-white shadow-sm text-amber-600' : 'text-slate-400'}`}
                    >Yearly</button>
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col justify-end min-h-0">
                {data.length > 0 ? (
                    <>
                        <div className="flex-1 flex items-end gap-4 border-b border-slate-100 pb-2 min-h-0">
                            {data.map((d, i) => {
                        ...
                        <div className="mt-8 flex justify-center gap-4 flex-wrap">
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-[9px] font-bold text-slate-500 uppercase tracking-tighter">High (3k+)</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                                <span className="w-2.5 h-2.5 rounded-xs bg-yellow-500"></span>
                                <span className="text-[9px] font-bold text-slate-500 uppercase tracking-tighter">Med (1k-3k)</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                                <span className="w-2.5 h-2.5 rounded-xs bg-red-500"></span>
                                <span className="text-[9px] font-bold text-slate-500 uppercase tracking-tighter">Low (&lt;1k)</span>
                            </div>
                        </div>
                    </>
                ) : (
                    <div className="flex items-center justify-center flex-1 text-slate-400 italic text-sm">No value rating data available.</div>
                )}
            </div>
        </div>
    );
