...
const CustomerTable: React.FC<CustomerTableProps> = ({ 
  ...
  const CustomSortHeader = ({ field, label, align = 'left' }: { field: string, label: string, align?: 'left' | 'right' | 'center' }) => (
    <th 
        ...
        <div className={`overflow-x-auto ${isLoading ? 'opacity-50 pointer-events-none transition-opacity' : ''}`}>
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50">
              <tr>
                <CustomSortHeader field="name" label="Customer" />
                <CustomSortHeader field="lastPurchaseDate" label="Last Activity" />
                {!hideInteractionColumn && <CustomSortHeader field="lastInteractionDate" label="Last Interaction" />}
                <CustomSortHeader field="valueRating" label="Status" />
                <CustomSortHeader field="totalSpending" label="Revenue" align="right" />
